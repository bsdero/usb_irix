...
hcd_methods_t ehci_methods={
	{ 1, "EHCI", "Enhanced Host Controller Interface (USB 2.0)", "usbehci.o", 
...
int register_ehci_with_usbcore(void *hcd){
	...
	rc = usbcore->register_hcd_driver( (void *) &ehci_methods);
...
int usbehci_attach(vertex_hdl_t conn){
    ...
    for( i = 0; ehci_descriptions[i].device_id != 0; i++){
        if( ehci_descriptions[i].device_id == device_vendor_id){
            ehci_methods.module_header.vendor_id = vendor_id;
            ehci_methods.module_header.device_id = device_id;
            strcpy( ehci_methods.module_header.hardware_description, 
